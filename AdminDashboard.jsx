...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    // Fetch user and vehicle counts
    const fetchCounts = async () => {
      try {
        // Replace these URLs with your actual API endpoints
        const userResponse = await fetch("/api/users/count");
        const vehicleResponse = await fetch("/api/vehicles/count");
    ...
    >
      ...
      <main className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div
            className={`p-4 rounded shadow ${theme === "dark" ? "bg-gray-800 text-white" : "bg-gray-100 text-black"}`}
          >
